...
The infinitesimal coefficient indicates residuals have no linear relationship with fitted values. Thus we can say how fitted values vary do not affect the residuals linearly.
...
If we consider discrimination with respect to gender, age and ethnic group size, those variables are not statistically significant to conclude discrimination exists.
...
From the anova tables below, we can see that the sum of the squared residuals decreases as the number of predictors increases, since the variance of the response variables is explained by the predictors. Also, the larger the variance one predictor explains, the more likely the predictor would be statistically significant. 
